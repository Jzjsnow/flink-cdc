...
/** converter {@link RecordData} type object to hudi field. */
public class HudiRowConverter implements Serializable {
    private static final long serialVersionUID = 1L;
    private static final ObjectMapper objectMapper = new ObjectMapper();

    /** Runtime converter to convert {@link RecordData} type object to hudi field. */
    @FunctionalInterface
    ...
    static SerializationConverter createExternalConverter(DataType type, ZoneId pipelineZoneId) {
        switch (type.getTypeRoot()) {
            ...
            case MAP:
                return (index, val) -> val.getMap(index);
            case ROW:
                return (index, val) ->
